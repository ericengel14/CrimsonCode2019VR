...
public class mazegen : MonoBehaviour
{
        ...
        for (int i = 0; i <= Maze.GetUpperBound(0); i++)
        {
            for (int j = 0; j <= Maze.GetUpperBound(1); j++)
            {
                if (Maze[i, j] == 1)
                {
                    ...
                    ptype.transform.localScale += new Vector3(scale, scale*3, scale);
    ...
    private void RemoveBlocks()
    {
        ...
        int oneToEvelven = rnd.Next(1, width - 2);
        Maze[oneToEvelven, 0] = 0;
        Maze[oneToEvelven + 1, 0] = 0;
        oneToEvelven = rnd.Next(1, 10);
        Maze[oneToEvelven, width - 1] = 0;
        Maze[oneToEvelven + 1, width - 1] = 0;
        /*
        // entrance
        mod = Maze[0, 0]; index = 1;
        Maze[1, 0] = 0;
        Maze[0, 0] = 0;
        Maze[0, 1] = 0;
        Maze[0, 0] = 0;
